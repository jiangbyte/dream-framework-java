...
public class AuthsGroupServiceImpl extends ServiceImpl<AuthsGroupMapper, AuthsGroup> implements AuthsGroupService {
    ...
    public Page<AuthsGroup> page(AuthsGroupPageQuery req) {
        ...
        // 2. 获取当前用户权限信息
        List<String> roleAssignGroupIds = AuthInfoUtil.getRoleAssignGroupIds(); // 自定义授权组ID
        List<String> roleDataScopes = AuthInfoUtil.getRoleDataScopes();         // 数据权限类型
        List<String> userGroupIds = AuthInfoUtil.getGroupIds();                 // 用户所属组ID
    ...
    public void add(AuthsGroupDto req) {
        ...
        if (bean.getPid() == null || bean.getPid().equals("0")) {
            ...
            String newAncestors;
            if (parent.getPidPath() == null) {
                newAncestors = "0" + "," + parent.getId();
            } else {
                newAncestors = parent.getPidPath() + "," + parent.getId();
            }
    ...
    public void edit(AuthsGroupDto req) {
        ...
        // 查询旧数据
        AuthsGroup old = this.getById(req.getId());
        // 新数据
        AuthsGroup updated = BeanUtil.toBean(req, AuthsGroup.class);

        // 判断 pid 是否发生变化
        boolean pidChanged = !Objects.equals(old.getPid(), updated.getPid());
        ...
        if (pidChanged) {
            ...
            if (updated.getPid() == null || "0".equals(updated.getPid())) {
                ...
                if (newParent.getPidPath() == null) {
                    newPidPath = "0" + "," + newParent.getId();
                } else {
    ...
    @Override
    public List<AuthsGroup> listTreeOptions(String keyword) {
        QueryWrapper<AuthsGroup> queryWrapper = new QueryWrapper<AuthsGroup>().checkSqlInjection();

        // 1. 关键词搜索
        if (ObjectUtil.isNotEmpty(keyword)) {
            queryWrapper.lambda().like(AuthsGroup::getName, keyword);
        }

        queryWrapper.lambda().orderByAsc(AuthsGroup::getSort);

        List<AuthsGroup> groups = this.list(queryWrapper);
        // 使用TreeBuilder构建树形结构
