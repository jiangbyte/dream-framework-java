...
async function loadPageData() {
  const { data } = await withLoading(useUserPreferenceApi().PageUserPreference(pageParams))
  pageData.value = data
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  const { success } = await useUserPreferenceApi().DeleteUserPreference(idArray)
  if (success) {
    loadPageData()
  }
}
