...
async function loadPageData() {
  withLoading(useAuthsGroupApi().PageAuthsGroup(pageParams)).then(({ data }) => {
    pageData.value = data
  })
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  useAuthsGroupApi().DeleteAuthsGroup(idArray).then(({ success }) => {
    if (success) {
      loadPageData()
    }
  })
  ...
  <div class="flex">
    ...
    <div class="flex-1">
      ...
      >
        ...
        <template #maxUserCount="{ row }">
          <SortTag :value="row.maxUserCount" />
        </template>
