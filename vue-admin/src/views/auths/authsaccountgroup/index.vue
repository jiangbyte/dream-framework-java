...
async function loadPageData() {
  withLoading(useAuthsAccountGroupApi().PageAuthsAccountGroup(pageParams)).then(({ data }) => {
    pageData.value = data
  })
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  useAuthsAccountGroupApi().DeleteAuthsAccountGroup(idArray).then(({ success }) => {
    if (success) {
      loadPageData()
    }
  })
