...
async function loadPageData() {
  withLoading(useAuthsAccountRoleApi().PageAuthsAccountRole(pageParams)).then(({ data }) => {
    pageData.value = data
  })
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  useAuthsAccountRoleApi().DeleteAuthsAccountRole(idArray).then(({ success }) => {
    if (success) {
      loadPageData()
    }
  })
