...
async function loadPageData() {
  withLoading(useAuthsRoleMenuApi().PageAuthsRoleMenu(pageParams)).then(({ data }) => {
    pageData.value = data
  })
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  useAuthsRoleMenuApi().DeleteAuthsRoleMenu(idArray).then(({ success }) => {
    if (success) {
      loadPageData()
    }
  })
