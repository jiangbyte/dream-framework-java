...
async function loadPageData() {
  const { data } = await withLoading(useConfigGroupApi().PageConfigGroup(pageParams))
  pageData.value = data
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  const { success } = await useConfigGroupApi().DeleteConfigGroup(idArray)
  if (success) {
    loadPageData()
  }
}
