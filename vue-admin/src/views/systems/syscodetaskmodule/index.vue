...
async function loadPageData() {
  withLoading(useSysCodeTaskModuleApi().PageSysCodeTaskModule(pageParams)).then(({ data }) => {
    pageData.value = data
  })
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  useSysCodeTaskModuleApi().DeleteSysCodeTaskModule(idArray).then(({ success }) => {
    if (success) {
      loadPageData()
    }
  })
