...
async function loadPageData() {
  withLoading(useConfigsItemApi().PageConfigsItem(pageParams)).then(({ data }) => {
    pageData.value = data
  })
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  useConfigsItemApi().DeleteConfigsItem(idArray).then(({ success }) => {
    if (success) {
      loadPageData()
    }
  })
...
async function treeNodeClickHandler(value: Array<any>, context: any) {
  const { node } = context
  pageParams.groupCode = value[0] ? node.data.code : ''
  loadPageData()
}
</script>
